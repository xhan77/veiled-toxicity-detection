...
def main():
    ...
    for tmp_idx, (input_ids, input_mask, segment_ids, label_ids, guids) in enumerate(test_dataloader):
        ...
        model.eval()
        model.zero_grad()
        _, pooled_output = model.bert(input_ids, segment_ids, input_mask, output_all_encoded_layers=False)
        test_emb = pooled_output.view(-1)
        ################
        
        ihvp_dict[tmp_idx] = test_emb.detach().cpu() # put to CPU to save GPU memory
    ...
    for train_idx, (_input_ids, _input_mask, _segment_ids, _label_ids, _) in enumerate(tqdm(train_dataloader, desc="Train set index")):
        ...
        ######## L_TRAIN GRADIENT ########
        model.zero_grad()
        _, pooled_output = model.bert(_input_ids, _segment_ids, _input_mask, output_all_encoded_layers=False)
        train_emb = pooled_output.view(-1)
        ################
    
        with torch.no_grad():
            for tmp_idx in ihvp_dict.keys():
                influence_dict[tmp_idx][train_idx] = torch.dot(ihvp_dict[tmp_idx], train_emb).item()
